...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'add') {
        ...
        //Perbaikan Bug Validasi ID Kamar untuk Tambah Kamar
        if ($nomor === '') {
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" style="display:grid;grid-template-columns:repeat(5,1fr) auto;gap:.75rem;align-items:end;">
                ...
                <div class="form-group" style="margin:0;"><label>Nomor</label><input type="text" name="nomor" required value="<?= htmlspecialchars($editKamar['nomor'] ?? '') ?>"></div>
                ...
                <div class="form-group" style="margin:0;"><label>Harga/Bulan</label><input type="number" name="harga_bulan" required value="<?= $editKamar['harga_bulan'] ?? '' ?>" min="1"></div>
                <div class="form-group" style="margin:0;"><label>Fasilitas</label><input type="text" name="fasilitas" required value="<?= htmlspecialchars($editKamar['fasilitas'] ?? '') ?>"></div>
                <?php if ($editKamar): ?>
